...
class User(Base):
    ...
    productions: Mapped[list[Production]] = relationship("Production", back_populates="user", cascade="all, delete-orphan")
...
class ProductionRecipe(Base):
    ...
    production_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("productions.id", ondelete="CASCADE"), nullable=False, index=True
    )
